...
export function EmptyState({ label }) {
  return <div style={{ fontFamily: "'Sora', sans-serif", fontSize: '0.75rem', color: 'hsl(0 0% 30%)', padding: '40px 0' }}>No {label} data yet.</div>;
}

export default function EducationSection({ education }) {
  if (!education?.length) return null;
  return (
    <section id="education" className="cm-section" style={{ padding: '7rem 64px' }}>
        ...
        <div style={{ borderTop: '1px solid hsl(0 0% 18%)' }}>
          {education.map((edu, i) => (
            <motion.div key={edu.id || i}
              ...
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '28px 0', borderBottom: '1px solid hsl(0 0% 15%)', gap: 40 }}>
              <div style={{ flex: 1 }}>
                <div className="cm-num" style={{ marginBottom: 8 }}>{String(i + 1).padStart(2, '0')}</div>
                <h3 style={{ fontFamily: "'Sora', sans-serif", fontSize: '1.45rem', fontWeight: 400, color: 'hsl(0 0% 90%)', marginBottom: 6, letterSpacing: '-0.01em' }}>{edu.title || edu.degree}</h3>
                <p style={{ fontFamily: "'Sora', sans-serif", fontSize: '0.85rem', fontWeight: 300, color: 'hsl(0 0% 40%)' }}>{edu.institution}</p>
              </div>
